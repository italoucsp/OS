...
//resume <PID> -> reanuda el proceso indicado
//kill <PID> -> termina el proceso indicado
//stop <PID> -> detiene el proceso indicado
...
int readInput(char *commands[], char input[], int length) {
    ...
    if(!strcmp(commands[0],"stop")) {
        int pid_p = atoi(commands[1]);
        kill(pid_p, SIGSTOP);
        return 0;
    }

    if(!strcmp(commands[0],"resume")) {
        int pid_p = atoi(commands[1]);
        kill(pid_p, SIGCONT);
